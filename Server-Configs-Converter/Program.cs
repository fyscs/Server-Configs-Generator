...
namespace Kxnrl.FyS.ConfigConverter
{
    ...
    static class Program
    {
        ...
        private static void ParseConfig(string filePath)
        {
            ...
            {
                $"// This file was auto-generated by ZombiEscape - Map Configs (https://github.com/Kxnrl/ZombiEscape)",
            ...
            {
                // check convar?
                var dict = configs.FirstOrDefault(x => x.ConVar == conf.Key);
                if (dict == null)
                {
                    Console.ForegroundColor = ConsoleColor.Cyan;
                    Console.WriteLine($"Skip invalid convar [{conf.Key}]");
                    continue;
                }

                var part = dict.Clone();
                part.DefVal = conf.Value;
        ...
        static void PrepareConfigs()
        {
            Configs["ze"] = new List<Variable>
            {
                ...
                // Rank
                new Variable("ze_damage_zombie_cash", "0.4", "Rank", "伤害与金钱转化比例 ($)", "0.0", "30.0"),
                new Variable("ze_damage_rank_points", "30000", "Rank", "伤害云点转化比例 (云点)", "5000.0", "999999.0"),
                new Variable("ze_damage_shop_credit", "50000", "Rank", "伤害积分转化比例 (积分)", "5000.0", "999999.0"),
                new Variable("ze_credits_pass_round", "1", "Rank", "通关所获得的积分 (积分)", "0", "30"),
                new Variable("rank_ze_win_points_humans", "", "Rank", "通关获得多少云点<人类>", "0", "30"),
                ...
                new Variable("ze_grenade_nade_cfthrust", "400", "Grenade", "高爆击退模式的击退力度 (Unit)", "0.0", "9999.9"),
                ...
                new Variable("ze_weapons_awp_counts", "5", "Weapon", "每局最大可购买的Awp数量 (把)", "0", "64"),
                new Variable("ze_weapons_spawn_hegrenade", "1", "Weapon", "每局开始时补给的高爆数量 (个)", "0", "10"),
                new Variable("ze_weapons_spawn_molotov", "0", "Weapon", "每局开始时补给的火瓶数量 (个)", "0", "10"),
                new Variable("ze_weapons_spawn_decoy", "1", "Weapon", "每局开始时补给的冰冻数量 (个)", "0", "10"),
                ...
                new Variable("sm_hunter_leappower", "300.0", "ZSkill", "闪灵冲刺推力 (Unit)", "1.0", "500.0"),
